...
document.getElementById('survey').addEventListener('submit', (e) => {
    ...
    fetch(`assets/php/cookiedata.php?cookiename=logintoken`).then((response) => response.text()).then((res) => {
        ...
        if (res != null) {
            ...
            document.getElementById('errorText').innerHTML = "You must have an account to save. <a href='profile'>Click here</a>";
        }
        if (loggedIn == "true") {
            ...
            document.getElementById('errorText').innerHTML = "You must have an account to save. <a href='profile'>Sign up</a>";
        }
    });
})